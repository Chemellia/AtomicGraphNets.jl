#=
 First basic MWE for graph AD. Mostly based on example 1, but with the graph-building and featurization tacked on the front. Also currently no force data for training because Materials Project doesn't have it.
=#
...
using Xtals
rc[:paths][:crystals] = @__DIR__ # so Xtals knows where to find files

function train_total_energy(;
    num_pts = 100,
    num_epochs = 5,
    ...
    # next, read in the structures
    if verbose
        println("Reading in Crystal objects from files...")
    end
    cifpaths = data_dir * prop * "_cifs/" .* info[!, Symbol(id)] .* ".cif"
    inputs = Crystal.(cifpaths)
